...
__global__ void KernelA(const TKparams Kparams)
{
    ...
    u16* __restrict__ lds_jlist = (u16*)&LDS[8 * JMP_CNT]; //4KB, must be aligned 16bytes
    ...
    u64 dp_mask64 = (Kparams.DP == 0) ? ~0ull : ~((1ull << (64 - Kparams.DP)) - 1);
    u16 jmp_ind;
        ...
        while (group != g_end)
        {
            ...
            else
            {
                ...
                else //no s(-1), need to calc it
                {
                    ...
                    u32 jmp_tmp = x0_cache[0] & JMP_MASK;
...
__global__ void KernelB(const TKparams Kparams)
{
    ...
    for (u32 gr_ind2 = 0; gr_ind2 < PNT_GROUP_CNT/2; gr_ind2++)
    {
        ...
        {
            RegsA[i2] = Kparams.LoopTable[MD_LEN * BLOCK_SIZE * PNT_GROUP_CNT * BLOCK_X + 2 * MD_LEN * BLOCK_SIZE * gr_ind2 + i2 * BLOCK_SIZE + THREAD_X];
            RegsB[i2] = Kparams.LoopTable[MD_LEN * BLOCK_SIZE * PNT_GROUP_CNT * BLOCK_X + 2 * MD_LEN * BLOCK_SIZE * gr_ind2 + (i2 + MD_LEN) * BLOCK_SIZE + THREAD_X];
        }
        u32 cur_indA = 0;
        u32 cur_indB = 0;
        ...
        for (int i2 = 0; i2 < MD_LEN; i2++)
        {
            int ind2 = (i2 + MD_LEN - cur_indA) % MD_LEN;
            Kparams.LoopTable[MD_LEN * BLOCK_SIZE * PNT_GROUP_CNT * BLOCK_X + 2 * MD_LEN * BLOCK_SIZE * gr_ind2 + ind2 * BLOCK_SIZE + THREAD_X] = RegsA[i2];
            ind2 = (i2 + MD_LEN - cur_indB) % MD_LEN;
            Kparams.LoopTable[MD_LEN * BLOCK_SIZE * PNT_GROUP_CNT * BLOCK_X + 2 * MD_LEN * BLOCK_SIZE * gr_ind2 + (ind2 + MD_LEN) * BLOCK_SIZE + THREAD_X] = RegsB[i2];
        }
    }
}
...
cudaError_t cuSetGpuParams(TKparams Kparams, u64* _jmp2_table)
{
    ...
    // preferire shared per KernelA/C (grandi tabelle in LDS)
    cudaFuncSetCacheConfig(KernelA, cudaFuncCachePreferShared);
    cudaFuncSetCacheConfig(KernelC, cudaFuncCachePreferShared);
