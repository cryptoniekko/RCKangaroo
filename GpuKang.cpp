...
//allocate gpu mem
    ...
    size = (u64)mpCnt * Kparams.BlockSize * (IsOldGpu ? sizeof(u64) : sizeof(u32));
    total_mem += size;
    ...
    if (cudaHostAlloc((void**)&DPs_out, MAX_DP_CNT * GPU_DP_SIZE, cudaHostAllocDefault) != cudaSuccess) {
        // fallback se la host-pinned fallisce
...
void RCGpuKang::Release()
{
    ...
    if (DPs_out) {
        cudaError_t _e = cudaFreeHost(DPs_out);
        if (_e != cudaSuccess) free(DPs_out);
    }
...
/**/
    ...
    err = cudaMemset(Kparams.L1S2, 0, (size_t)mpCnt * Kparams.BlockSize * (IsOldGpu ? 8 : 4));
    if (err != cudaSuccess)
        return false;
